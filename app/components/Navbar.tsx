...
const NAV_ITEMS = [
    { label: 'Home', href: '/' },
    {
        ...
        groups: [
            {
                heading: 'Property Types', links: [
                    ...
                    { label: 'New Projects', href: '/projects', Icon: IcCrane },
        ...
        featured: {
            label: 'New Launches',
            sub: 'Discover premium projects before they sell out.',
            href: '/projects',
            img: 'https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=360&q=80',
        },
    },
    ...
    { label: 'Sell', dropdown: [{ label: 'Sell Your Property', href: '#' }, { label: 'Free Property Valuation', href: '#' }, { label: 'Seller Guide', href: '#' }] },
    {
        ...
        dropdown: [
            ...
            { label: 'New Launches', href: '/projects?status=Available' },
            { label: 'Under Construction', href: '/projects?status=Under+Construction' },
            { label: 'Investment Opportunities', href: '/projects?type=Residential' },
        ],
    },
    {
        label: 'About', href: '/about',
        dropdown: [
            { label: 'Company Overview', href: '/about#au-intro' },
            { label: 'Mission & Vision', href: '/about#au-values' },
            { label: 'Meet the Team', href: '/about#au-team' },
            { label: 'Testimonials', href: '/about#au-testimonials' },
        ],
    },
    {
...
:root{
  ...
  --nv-utility-h:32px;--nv-main-h:70px;--nv-total-h:102px;--nv-mobile-h:64px;
}
...
.nv.scrolled .nv-main{
  ...
  box-shadow:0 1px 0 var(--g200),0 4px 24px rgba(0,0,0,0.05);height:60px;
}

/* ── utility bar ── */
.nv-utility{
  display:flex;justify-content:flex-end;align-items:center;gap:1.25rem;
  padding:0.4rem 6%;background:var(--g900);height:var(--nv-utility-h);
}
.nv-util-link{
  display:inline-flex;align-items:center;gap:0.4rem;font-size:0.74rem;font-weight:500;
...
.nv-util-lang{
  ...
  font-family:'Inter',sans-serif;cursor:pointer;transition:color 0.18s;padding:0;position:relative;
}
.nv-util-lang:hover{color:#fff;}
.nv-lang-dd{
  position:absolute;top:calc(100% + 10px);right:0;background:#fff;
  border:1px solid var(--g200);border-radius:12px;padding:0.3rem;
  box-shadow:0 12px 40px rgba(0,0,0,0.12);min-width:140px;z-index:300;
...
.nv-lang-opt{
  ...
  padding:0.55rem 0.75rem;border-radius:8px;cursor:pointer;border:none;background:none;
  width:100%;font-family:'Inter',sans-serif;font-size:0.8rem;font-weight:500;
  color:var(--g700);transition:background 0.12s,color 0.12s;text-align:left;
}
...
.nv-main{
  ...
  padding:0 6%;height:var(--nv-main-h);background:#fff;
  transition:height 0.3s,background 0.3s,box-shadow 0.3s;gap:1rem;
}
...
.nv-logo-name{
  ...
  color:var(--g900);letter-spacing:-0.03em;line-height:1;transition:color 0.18s;
}
...
.nv-btn{
  display:inline-flex;align-items:center;gap:0.25rem;padding:0.45rem 0.6rem;
  border:none;background:none;cursor:pointer;font-family:'Inter',sans-serif;
  font-size:0.8rem;font-weight:500;color:var(--g600);border-radius:8px;
...
/* active underline for Projects link */
.nv-btn[href="/projects"]{position:relative;}
.nv-btn[href="/projects"]::after{
  content:'';position:absolute;bottom:-2px;left:50%;
  transform:translateX(-50%);width:18px;height:2px;
  background:var(--g900);border-radius:2px;opacity:0;transition:opacity 0.2s;
}
.nv-btn[href="/projects"]:hover::after{opacity:1;}
...
.nv-dd{
  ...
  transform:translateX(-50%) translateY(4px);background:#fff;
  border:1px solid var(--g200);border-radius:14px;padding:0.4rem;
  box-shadow:0 16px 48px rgba(0,0,0,0.09),0 4px 14px rgba(0,0,0,0.04);
  min-width:210px;z-index:200;opacity:0;pointer-events:none;
  transition:opacity 0.15s,transform 0.15s;
}
.nv-item:hover .nv-dd{opacity:1;pointer-events:auto;transform:translateX(-50%) translateY(0);}
.nv-dd-link{
  display:block;padding:0.575rem 0.8rem;border-radius:8px;text-decoration:none;
  font-size:0.825rem;font-weight:500;color:var(--g700);
  transition:background 0.12s,color 0.12s;white-space:nowrap;
}
.nv-dd-link:hover{background:var(--g50);color:var(--g900);}

/* Projects dropdown — pill badge on "All Projects" */
.nv-dd-link-badge{
  display:flex;align-items:center;justify-content:space-between;gap:8px;
}
.nv-dd-badge{
  font-size:0.56rem;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;
  background:var(--g900);color:#fff;padding:2px 7px;border-radius:999px;flex-shrink:0;
}
...
.nv-mega{
  position:absolute;top:calc(100% + 14px);left:50%;
  transform:translateX(-50%) translateY(4px);background:#fff;
  border:1px solid var(--g200);border-radius:18px;padding:1.375rem;
  box-shadow:0 20px 64px rgba(0,0,0,0.09),0 4px 18px rgba(0,0,0,0.04);
  width:660px;display:grid;grid-template-columns:1fr 1fr 1.05fr;
  gap:1.375rem;z-index:200;opacity:0;pointer-events:none;
  transition:opacity 0.15s,transform 0.15s;
}
...
.mega-head{
  ...
  color:var(--g400);margin-bottom:0.5rem;padding:0 0.5rem;
}
.mega-link{
  display:flex;align-items:center;gap:0.6rem;padding:0.45rem 0.6rem;
  border-radius:8px;text-decoration:none;font-size:0.8125rem;
  font-weight:500;color:var(--g700);transition:background 0.12s,color 0.12s;
}
.mega-link:hover{background:var(--g50);color:var(--g900);}
.mega-ic{
  width:28px;height:28px;border-radius:7px;background:var(--g100);
...
.mega-feat{
  border-radius:12px;overflow:hidden;border:1px solid var(--g100);
...
.mega-feat-eye{
  ...
  color:rgba(255,255,255,0.35);margin-bottom:0.2rem;
}
.mega-feat-title{
  font-family:'Outfit',sans-serif;font-size:0.9rem;font-weight:700;color:#fff;
  letter-spacing:-0.02em;margin-bottom:0.3rem;line-height:1.3;
}
...
.mega-feat-cta{
  ...
  font-size:0.72rem;font-weight:700;color:rgba(255,255,255,0.6);transition:color 0.15s;
}
...
.nv-icon{
  position:relative;width:36px;height:36px;border:none;background:none;
  border-radius:9px;display:flex;align-items:center;justify-content:center;
  cursor:pointer;color:var(--g600);transition:background 0.15s,color 0.15s;
}
...
.nv-btn-out{
  padding:0.44rem 0.9rem;border:1.5px solid var(--g200);border-radius:9px;
  background:none;font-family:'Inter',sans-serif;font-size:0.8rem;font-weight:600;
  color:var(--g700);cursor:pointer;transition:all 0.15s;white-space:nowrap;
}
.nv-btn-out:hover{border-color:var(--g700);color:var(--g900);background:var(--g50);}
.nv-btn-prim{
  padding:0.44rem 1rem;border:none;border-radius:9px;
  background:linear-gradient(135deg,#18181b,#27272a);
  font-family:'Inter',sans-serif;font-size:0.8rem;font-weight:700;color:#fff;
  cursor:pointer;white-space:nowrap;box-shadow:0 2px 8px rgba(0,0,0,0.18);
...
.nv-sp{
  position:absolute;top:calc(100% + 10px);right:0;background:#fff;
  border:1px solid var(--g200);border-radius:14px;padding:0.875rem;
  box-shadow:0 16px 48px rgba(0,0,0,0.1);width:500px;
  display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:0.5rem;
  align-items:stretch;opacity:0;pointer-events:none;
...
.sf{
  display:flex;flex-direction:column;gap:2px;padding:0.45rem 0.7rem;
  background:var(--g50);border-radius:8px;border:1.5px solid transparent;
...
.sf select,.sf input{
  border:none;background:transparent;font-size:0.8rem;color:var(--g900);
  font-weight:500;font-family:'Inter',sans-serif;outline:none;width:100%;
  -webkit-appearance:none;appearance:none;
}
.sf input::placeholder{color:var(--g400);}
.sg{
  padding:0 0.9rem;background:var(--g900);border:none;border-radius:8px;
  color:#fff;cursor:pointer;display:flex;align-items:center;gap:0.375rem;
...
.ham-l{
  width:20px;height:2px;background:var(--g900);border-radius:2px;
  transition:transform 0.28s,opacity 0.28s,width 0.28s;transform-origin:center;
}
...
.nv-dr{
  ...
  display:flex;flex-direction:column;box-shadow:-16px 0 48px rgba(0,0,0,0.1);
}
.nv-dr.open{right:0;}
.mob-hd{
  display:flex;align-items:center;justify-content:space-between;
  padding:1.125rem 1.25rem;border-bottom:1px solid var(--g100);
  position:sticky;top:0;background:#fff;z-index:1;
}
.mob-hd-tag{font-size:0.68rem;font-weight:700;letter-spacing:0.13em;text-transform:uppercase;color:var(--g400);}
.mob-cl{
  width:34px;height:34px;border:none;background:var(--g100);border-radius:8px;
...
.mob-srch{
  margin:0.875rem 1.25rem;display:flex;align-items:center;gap:0.5rem;
  background:var(--g50);border:1.5px solid var(--g200);border-radius:9px;padding:0.6rem 0.875rem;
}
.mob-srch input{
  border:none;background:none;font-size:0.875rem;color:var(--g900);
...
.mob-lbtn{
  ...
  color:var(--g800);text-align:left;text-decoration:none;transition:background 0.12s;
}
...
.mob-sa{
  display:block;padding:0.625rem 1.25rem 0.625rem 1.875rem;text-decoration:none;
  font-size:0.85rem;font-weight:500;color:var(--g600);
  border-bottom:1px solid var(--g100);transition:color 0.12s,background 0.12s;
}
.mob-sa:last-child{border-bottom:none;}
.mob-sa:hover{color:var(--g900);background:var(--g100);}
.mob-ft{
  padding:1.125rem 1.25rem;border-top:1px solid var(--g100);
  display:flex;flex-direction:column;gap:0.5rem;position:sticky;bottom:0;background:#fff;
}
.mob-cp{
  width:100%;padding:0.8rem;background:var(--g900);border:none;border-radius:10px;
  font-family:'Inter',sans-serif;font-size:0.9rem;font-weight:700;color:#fff;
  cursor:pointer;transition:background 0.15s;
}
.mob-cp:hover{background:var(--g700);}
.mob-cs{
  width:100%;padding:0.8rem;background:none;border:1.5px solid var(--g200);
  border-radius:10px;font-family:'Inter',sans-serif;font-size:0.9rem;font-weight:600;
            ...
            <div className={`nv ${isScrolled ? 'scrolled' : ''}`} ref={navRef}>
                    ...
                    <div className="nv-utility">
                        ...
                        >
                            ...
                            <div className={`nv-lang-dd ${langOpen ? 'open' : ''}`}>
                                <button
                                    className={`nv-lang-opt ${lang === 'EN' ? 'active' : ''}`}
                                    onClick={e => { e.stopPropagation(); setLang('EN'); setLangOpen(false); }}
                                >
                                    <span>English</span>
                                ...
                                <button
                                    className={`nv-lang-opt ${lang === 'AR' ? 'active' : ''}`}
                                    onClick={e => { e.stopPropagation(); setLang('AR'); setLangOpen(false); }}
                                >
                                    <span>العربية</span>
                ...
                <div className="nv-main">
                                    ...
                                    {item.dropdown && (
                                        <div className="nv-dd">
                                            {item.dropdown.map((link, li) => (
                                                <a key={link.label} href={link.href} className="nv-dd-link">
                                                    {/* Badge on "All Projects" */}
                                                    {item.label === 'Projects' && li === 0 ? (
                                                        <span className="nv-dd-link-badge">
                                                            {link.label}
                                                            <span className="nv-dd-badge">New</span>
                                                        </span>
                                                    ) : link.label}
                                                </a>
                                            ))}
                    ...
                    <div className="nv-right">
                        {/* Search */}
                        <div className="nv-sw">
                            <button className="nv-icon" onClick={() => setSearchOpen(o => !o)} aria-label="Search">
                                <IcSearch />
                            </button>
                            <div className={`nv-sp ${searchOpen ? 'open' : ''}`}>
                                ...
                                <div className="sf">
                                    ...
                                    <select><option>Any Type</option><option>Apartment</option><option>Villa</option><option>Penthouse</option></select>
                                </div>
                                <div className="sf">
                                    <span className="sf-label">Price</span>
                                    <select><option>Any Price</option><option>Under $500K</option><option>$500K-$1M</option><option>$1M-$3M</option><option>$3M+</option></select>
                                </div>
                                <button className="sg">
                        ...
                        {/* Hamburger */}
                        <button
                            className={`nv-ham ${mobileOpen ? 'open' : ''}`}
                                ...
                                {hasSub ? (
                                    <>
                                        ...
                                        <div className={`mob-sub ${isExp ? 'open' : ''}`}>
                                            {allLinks.map(link => (
                                                <a key={link.label} href={link.href} className="mob-sa">
                                                    {link.label}
                                                </a>
                                            ))}
                                        </div>
